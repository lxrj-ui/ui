...
function SelectTrigger({ className, children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button
      ...
      className={cn(
        "group/trigger flex h-9 w-full items-center justify-between rounded-md border bg-[var(--color-input-bg)] px-3 py-2 text-sm shadow-xs focus-visible:outline-none focus-visible:border-[var(--focus-border)] focus-visible:shadow-[var(--focus-shadow)] disabled:opacity-50",
        className
      )}
    ...
    >
      ...
      <div className="relative shrink-0">
        <ChevronDown className="h-4 w-4 opacity-50 transition-all duration-200" />
        <span className="pointer-events-none absolute inset-0 rounded-sm border opacity-0 transition-opacity duration-200 hover:opacity-100 group-hover/trigger:opacity-100" style={{ borderColor: "var(--color-border)" }} />
      </div>
    </button>
  );
}
